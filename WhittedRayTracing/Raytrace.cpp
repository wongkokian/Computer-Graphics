...
// Take the nearest hit point.
    ...
    //**********************************
    // REMOVE THIS LINE AFTER YOU HAVE FINISHED CODE BELOW.
    //**********************************
...
// Compute for shadow if hasShadow is true.
    ...
    for (int i = 0; unsigned(i) < scene.ptLights.size(); i++) {
        Vector3d L = scene.ptLights[i].position - nearestHitRec.p;
        Ray ray = Ray(nearestHitRec.p, L.unitVector());
        bool blocked = false;

        if (hasShadow) {
            for (int j = 0; unsigned(j) < scene.surfaces.size(); j++) {
                if (scene.surfaces[j]->shadowHit(ray, DEFAULT_TMIN, L.length())) {
                    blocked = true;
                    break;
                }
            }
        ...
        if (!blocked) {
            result += computePhongLighting(L.unitVector(), N, V, nearestHitRec.material, scene.ptLights[i]);
        }
...
// Add to result the reflection of the scene.
    ...
    if (reflectLevels != 0) {
        return result + nearestHitRec.material.k_rg * Raytrace::TraceRay(
            Ray(nearestHitRec.p, mirrorReflect(-uRay.direction(), N)), scene, reflectLevels - 1, hasShadow);
    }
