...
#define LOWER_BOX_Z_POS     CAR_HEIGHT / 4
#define UPPER_BOX_Z_POS     3 * (CAR_HEIGHT / 4)
#define UPPER_BOX_LENGTH    CAR_LENGTH / 2
#define BOX_HEIGHT          CAR_HEIGHT / 2

#define WHEEL_OUTER_RAD     CAR_HEIGHT / 4
#define WHEEL_INNER_RAD     WHEEL_OUTER_RAD / 2
#define WHEEL_N_SIDES       20
#define WHEEL_RINGS         20

#define WHEEL_X_POS         CAR_LENGTH / 4
#define WHEEL_Y_POS         (CAR_WIDTH / 2 + WHEEL_INNER_RAD)
#define WHEEL_Z_POS         LOWER_BOX_Z_POS

#define DEG_TO_RAD          PI / 180

#define DESIRED_MS          1000 / DESIRED_FPS
...
void DrawOneCar( float bodyColor[3] )
{
    ...
    // Draw lower box
    glPushMatrix();
    glTranslated(0, 0, LOWER_BOX_Z_POS);
    ...
    // Draw upper box
    glPushMatrix();
    glTranslated(0, 0, UPPER_BOX_Z_POS);
    glScaled(UPPER_BOX_LENGTH, CAR_WIDTH, BOX_HEIGHT);
    glutSolidCube(1);
    glPopMatrix();
    ...
    for (int i = 0; i < 4; i++) {
        ...
        if (i == 0) {
            glTranslated(WHEEL_X_POS, WHEEL_Y_POS, WHEEL_Z_POS);
        }
        else if (i == 1) {
            glTranslated(-WHEEL_X_POS, WHEEL_Y_POS, WHEEL_Z_POS);
        }
        ...
        glRotated(90, 1, 0, 0);
        glutSolidTorus(WHEEL_INNER_RAD, WHEEL_OUTER_RAD, WHEEL_N_SIDES, WHEEL_RINGS);
        glPopMatrix();
    }
}
...
void MyDisplay( void )
{
    ...
    gluLookAt( eyeDistance * sin(eyeLongitude * DEG_TO_RAD) * cos(eyeLatitude * DEG_TO_RAD),
        eyeDistance * sin(eyeLatitude * DEG_TO_RAD),
        eyeDistance * cos(eyeLongitude * DEG_TO_RAD) * cos(eyeLatitude * DEG_TO_RAD)
...
void MyTimer( int v )
{
    ...
    {
        UpdateCars();
        glutTimerFunc(DESIRED_MS, MyTimer, 0);
    }
}
